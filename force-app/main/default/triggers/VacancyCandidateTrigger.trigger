trigger VacancyCandidateTrigger on VacancyCandidate__c(
  before update,
  before insert,
  after update
) {
  ...
  if (Trigger.isBefore && Trigger.isInsert) {
    for (VacancyCandidate__c vacCan : Trigger.new) {
      vacCan.UniqueID__c =
        String.valueOf(vacCan.Vacancy__c) + String.valueOf(vacCan.Candidate__c);
    }
  }
  ...
  if (Trigger.isBefore && Trigger.isUpdate) {
    ...
    if (approvedVacCan.size() > 0) {
      approvedVacCan = VacancyCandidateHelper.filterValidateDuplicates(
        approvedVacCan
      );
    }
  ...
  if (Trigger.isAfter && Trigger.isUpdate) {
    ...
    for (VacancyCandidate__c vacCan : Trigger.new) {
      if (
        !usedVacanciesId.contains(vacCan.Vacancy__c) &&
        (vacCan.Processed__c && !Trigger.old.get(i).Processed__c)
      ) {
